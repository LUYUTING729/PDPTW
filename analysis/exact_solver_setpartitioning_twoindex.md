...
| `void createSP(std::string file_name)` | `file_name`：列文件 | 清空旧模型、重新读取列，按列成本创建 Gurobi 连续变量（车辆受限时系数为原成本，否则加 10000 罚项），并为每个节点添加覆盖约束及（可选）车辆数约束。 | 刷新 `model`、`spVars`、`spConstrs`，并导出 `rmp.lp` 供调试。【F:domain/solver/exact/SetPartitioning.cpp†L58-L107】 |
| `void solveSP()` | - | 调用 `createSP(cf)` 构建模型，`model->optimize()` 求解。随后读取每个节点覆盖约束的对偶值填充 `rho_k`，并输出当前目标值。 | 更新 `rho_k`、保留 RMP 最优解供下一轮定价使用。【F:domain/solver/exact/SetPartitioning.h†L81-L102】 |
...
- `pricingSubProblem`：标记当前是否作为定价子问题运行；若为真，会调整目标系数、改用单路径流量约束并限制最大取货节点数。
- `pricingSubProblemData`：当 `pricingSubProblem = true` 时提供对偶值及取货节点数量上限。
...
3. **变量定义**：
   - `arcVariables[i][j]`：二进制弧变量 `x_ij`，根据可行弧筛选上下界，目标系数为行驶距离；若为定价子问题，将对应取货节点的对偶值从目标中扣除；若为原问题且 `i=0`，额外加 10000 以惩罚新增车辆。【F:domain/solver/exact/TwoIndexFormulation.cpp†L209-L273】
   - `pathVertexVariables`：当处理定价子问题时引入的 0-1 变量 `z_i`，用于限制选入路径的节点数。【F:domain/solver/exact/TwoIndexFormulation.cpp†L274-L285】
   - `startServiceTimeVariables` (`b_i`)：每个节点的服务开始时间，范围取节点时间窗，并在非 depot 情况下以 0 目标系数创建；`i=0` 的时间被固定在窗口下界。【F:domain/solver/exact/TwoIndexFormulation.cpp†L286-L312】
   - `vehicleLoadVariables` (`q_i`)：车辆载荷变量，范围由节点需求与车辆容量决定。【F:domain/solver/exact/TwoIndexFormulation.cpp†L313-L330】
   - `routeIdentifierVariables` (`v_i`)：连续变量标识节点所属路线首节点 ID，用于断开子回路并维系取送对的同路线约束。【F:domain/solver/exact/TwoIndexFormulation.cpp†L331-L343】
4. **流量约束**：
   - 若是定价子问题，使用流量平衡（入度 = 出度）并结合 `z_i` 变量保证单条路径；
   - 否则分别固定每个节点的入度和出度为 1，确保车辆路径覆盖所有节点一次。【F:domain/solver/exact/TwoIndexFormulation.cpp†L344-L372】
5. **时间窗（MTZ）约束**：对任意可行弧 `(i, j)` 添加 `b_i - b_j + M * x_ij ≤ M - travelTime`，保证时间累积合法。【F:domain/solver/exact/TwoIndexFormulation.cpp†L373-L406】
6. **载荷约束**：类似地添加 `q_i - q_j + M * x_ij ≤ M - demand_j`，维持容量守恒并防止超载。【F:domain/solver/exact/TwoIndexFormulation.cpp†L407-L438】
7. **取送先后与同路线约束**：
   - 对每对取送节点加入 `b_pickup - b_delivery ≤ -travelTime`，确保先取后送；
   - `v_pickup = v_delivery` 强制送货节点与取货节点属于同一路线；
   - 结合 depot 出发弧的上下界，约束 `v_i` 的取值范围，进而在所有弧 `(i, j)` 上加上下界/上界不等式以消除子回路。【F:domain/solver/exact/TwoIndexFormulation.cpp†L439-L520】
8. **定价特定约束**：若 `pricingSubProblem = true`，额外约束：
   - 恰好一条路径离开/返回仓库；
   - 选入的取货节点数量不超过 `pickupNodeNumLimitInRoute`；
   - （可选）使用 `z_i` 控制路径长度。【F:domain/solver/exact/TwoIndexFormulation.cpp†L521-L547】
9. **初始解/暖启动**：
...
10. **回调与模型导出**：
    - 若求解原问题（`pricingSubProblem = false`），注册自定义 `CallBack` 类，在 `GRB_CB_MIPSOL` 事件中读取解向量、重构 `vrpSolution` 路径并写入 `resultFolder`；
    - 生成 `two_index_formulation[_pricing_sub_problem].lp` 以便调试。【F:domain/solver/exact/TwoIndexFormulation.cpp†L22-L170】【F:domain/solver/exact/TwoIndexFormulation.cpp†L669-L716】
